...
{
    if (CGlobalVariable::listensocket->TooManySockets() && !(socket && socket->IsConnected()) )
    {
...
void CUpDownClient::UDPReaskForDownload()
{
		...
		!CGlobalVariable::IsFirewalled() && !(socket && socket->IsConnected()) && !thePrefs.GetProxySettings().UseProxy)
	{
		if ( !HasLowID()) 
		{
			...
			CGlobalVariable::clientudp->SendPacket(response, GetIP(), GetUDPPort(), ShouldReceiveCryptUDPPackets(), GetUserHash());
			m_nTotalUDPPackets++;
		}
		else if (HasLowID() && GetBuddyIP() && GetBuddyPort() && HasValidBuddyID())
		{
			...
			theStats.AddUpDataOverheadFileRequest(response->size);
			CGlobalVariable::downloadqueue->AddUDPFileReasks();
			CGlobalVariable::clientudp->SendPacket(response, GetBuddyIP(), GetBuddyPort(), false, NULL);  // KAD does not supports obfuscation yet
			m_nTotalUDPPackets++;
		}
	}
