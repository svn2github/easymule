...
*	@return
*/
void CSharedFileList::AddShareFilesFromDir(CString strDir, bool bSubDirectories,CStringList& strListAdded,bool bAddOnlyInKnownFile,bool bOnlyAddFileInSubDir)
{
	if(!bOnlyAddFileInSubDir)
	{
		AddFilesFromDirectory(strDir,bAddOnlyInKnownFile);
		strDir.MakeLower();
	...
	if (bSubDirectories)
	{
		...
		while (bWorking)
		{
			...
			if (finder.IsDots() || finder.IsSystem() || !finder.IsDirectory())
				continue;
			AddShareFilesFromDir(strDir + finder.GetFileName(), true,strListAdded,bAddOnlyInKnownFile);
		}
		finder.Close();
	}
}
...
void CSharedFileList::FindSharedFiles()
{
	...
	AddShareFilesFromDir(tempDir, true,l_sAdded,true,true);
...
void CSharedFileList::AddFilesFromDirectory(const CString& rstrDirectory,bool bAddOnlyInKnownFile)
{
	CFileFind ff;
	...
	while (!end)
	{
		...
		//if(CGlobalVariable::filemgr.IsFileDownloading(strTempPath)) continue;
...
bool CSharedFileList::SafeAddKFile(CKnownFile* toadd, bool bOnlyAdd)
{
	...
	if(bAdded)
		//SendMessage(CGlobalVariable::m_hListenWnd, WM_SHAREDFILE_ADDFILE, 0, (LPARAM)toadd);
...
void CSharedFileList::FileHashingFinished(CKnownFile* file)
{
	...
	{
		SafeAddKFile(file);
		CGlobalVariable::knownfiles->SafeAddKFile(file);
	}
	else
	{
...
bool CSharedFileList::HashNewFile(const CString & strFilepath)
{
	int nPathEnd=strFilepath.ReverseFind('\\');
	CString strDirectory = strFilepath.Mid(0, nPathEnd);
	if (!IsHashing(strDirectory, strFilepath) && !thePrefs.IsTempFile(strDirectory, strFilepath))
	{
		UnknownFile_Struct* tohash = new UnknownFile_Struct;
...
bool CSharedFileList::IsSharedPath(CString strPath)
{
	...
	if (tempDir.Right(1)!=_T("\\"))
		tempDir+=_T("\\");
	if(tempDir.CompareNoCase(strPath)==0) return true;
