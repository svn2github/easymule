...
void CIEMonitor::ApplyChanges( void )
{
	...
	if(m_bFirstRun)
	{
		bNeedReg = CheckForUpdate(thePrefs.GetMuleDirectory(EMULE_EXECUTEABLEDIR) + _T("IE2EM.dll"));
	}
	if(m_bFirstRun || m_bIEMenu != thePrefs.GetAddToIEMenu() || m_bMonitor != thePrefs.GetMonitorLinks() || m_wLangID != thePrefs.GetLanguageID())
	{
	...
	if(m_bAlt != thePrefs.GetMonitorOnAlt())
	{
		m_bAlt = thePrefs.GetMonitorOnAlt();
		CRegKey regkey;
		regkey.Create(HKEY_CURRENT_USER, _T("Software\\easyMule"));
		regkey.SetDWORDValue(_T("MonitorOnAlt"), m_bAlt ? 1 : 0);
		regkey.Close();
	}
	m_bFirstRun = false;
}
