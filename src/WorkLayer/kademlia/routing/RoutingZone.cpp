						...
						if (::IsGoodIPPort(uhostIP, uUDPPort))
						{
							...
							if (CGlobalVariable::ipfilter->IsFiltered(uhostIP))
							{
								if (::thePrefs.GetLogFilteredIPs())
									AddDebugLogLine(false, _T("Ignored kad contact (IP=%s)--read known.dat -- - IP filter (%s)") , ipstr(uhostIP), ::CGlobalVariable::ipfilter->GetLastHit());
							}
							else
							{
...
void CRoutingZone::WriteFile()
{
	try
	{
		...
		if (file.Open(m_sFilename, CFile::modeWrite | CFile::modeCreate | CFile::typeBinary|CFile::shareDenyWrite, &fexp))
		{
			...
			GetBootstrapContacts(&listContacts, 512);
...
bool CRoutingZone::Add(const CUInt128 &uID, uint32 uIP, uint16 uUDPPort, uint16 uTCPPort, uint8 uVersion, bool bUpdate)
{
	...
	if (::IsGoodIPPort(uhostIP, uUDPPort))
	{
		if (!::CGlobalVariable::ipfilter->IsFiltered(uhostIP)) {
			...
			AddDebugLogLine(false, _T("Ignored kad contact (IP=%s) - IP filter (%s)"), ipstr(uhostIP), ::CGlobalVariable::ipfilter->GetLastHit());
	}
	else if (::thePrefs.GetLogFilteredIPs())
