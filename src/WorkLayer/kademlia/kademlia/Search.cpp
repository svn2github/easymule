		...
		case STOREFILE:
			{
				...
				if (pFile)
				{
					...
					/*if( CGlobalVariable::IsFirewalled() )
					{
						...
						if( CGlobalVariable::clientlist->GetBuddy() )
						{
...
void CSearch::ProcessResultFile(const CUInt128 &uAnswer, TagList *plistInfo)
{
	...
	switch( uType )
	{
		...
		case 5:
			...
			//theApp.emuledlg->kademliawnd->searchList->SearchRef(this);
			SendMessage( CGlobalVariable::m_hListenWnd,WM_KAD_SEARCHREF,0,(LPARAM)this ); 
			CGlobalVariable::downloadqueue->KademliaSearchFile(m_uSearchID, &uAnswer, &uBuddy, uType, uIP, uTCPPort, uUDPPort, uServerIP, uServerPort, byCryptOptions);
			break;
	}
}
...
void CSearch::ProcessResultKeyword(const CUInt128 &uAnswer, TagList *plistInfo)
{
	...
	CGlobalVariable::searchlist->KademliaSearchKeyword(m_uSearchID, &uAnswer, sName, uSize, sType, 8,
