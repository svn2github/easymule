...
CNatSocket::CNatSocket(CAsyncSocketEx* Parent) : m_Parent(Parent)
{
	...
	m_uAdvertisedWindow = MAX_BUF_SIZE;
...
void CNatSocket::SetConfig(sUserModeTCPConfig* UserModeTCPConfig)
{
	...
	//m_uAdvertisedWindow = m_UserModeTCPConfig->MaxSegmentSize; 
	//m_uCongestionWindow = m_uAdvertisedWindow;

	//m_uCongestionThreshold = m_UserModeTCPConfig->MaxUploadBufferSize; // slow start ;)
}
...
int CNatSocket::Send(const void* lpBuf, int nBufLen, int /*nFlags*/)
{
	...
	//  send new buffer entry
	POSITION pos=m_UploadBuffer.GetHeadPosition();
	for(int i = 0; pos && i < WIN_SIZE; i++)
	{
		sTransferBufferEntry * ent=m_UploadBuffer.GetNextValue(pos);
		//  it is not full, maybe fill it next cycle
		if(ent->uSize < (m_UserModeTCPConfig->MaxSegmentSize)/2)
		{
			break;
		}

		if(! ent->uSendCount && m_uAdvertisedWindow > ent->uSize)
		{
			SendBufferedSegment(ent);
			m_uAdvertisedWindow -= ent->uSize;
		}
	}
...
*/
	...
	if(size && pBuffer)
	{
		...
		T_TRACE("Window Size %d", m_uAdvertisedWindow);
	}
...
//	T_TRACE("%s: uSequenceNr=%u",__FUNCTION__, uSequenceNr);
	...
	else if(m_UploadBuffer.Lookup(/*UINT_MAX-*/uSequenceNr,BufferEntry))
	{
		...
		// --------------------------------
		//uint32 WindowIncrement = m_UserModeTCPConfig->MaxSegmentSize; // exponenial increase
		...
		//m_uCongestionWindow = min(m_uCongestionWindow + WindowIncrement, m_UserModeTCPConfig->MaxUploadBufferSize);
		...
		{
			DWORD dwNewCheck = GetTickCount()-BufferEntry->uSendTime;
			m_dwTimeToCheck = ((m_dwTimeToCheck*3)>>2) + (dwNewCheck>>2);
			if(m_dwTimeToCheck<24) m_dwTimeToCheck = 24;
			else m_dwTimeToCheck += 48;
	...
	{
		m_dwTimeToCheck += (m_dwTimeToCheck>>3);
		TRACE("new CHECK_TIME = %d, duplicated packet\n", m_dwTimeToCheck);
	}
	...
	if(! m_UploadBuffer.IsEmpty())
	{
		...
		//	if(! BufferEntry->uSendCount && m_uAdvertisedWindow > BufferEntry->uSize)
		//	{
		//		SendBufferedSegment(BufferEntry);
		//		m_uAdvertisedWindow -= BufferEntry->uSize;
		//	}
		//}
