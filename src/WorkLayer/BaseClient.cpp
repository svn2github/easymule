...
CUpDownClient::~CUpDownClient(){
	...
	if( CGlobalVariable::natthread /*&& this peer is NatTrans Peer*/ )
		CGlobalVariable::natthread->RemoveNatSocket( m_achUserHash );
...
{
	if (CGlobalVariable::listensocket->TooManySockets() && !bIgnoreMaxCon && !(socket && socket->IsConnected()))
	{
	...
	{
		//  Comment UI
		if(!CGlobalVariable::DoCallback(this))
		{
		...
		if( HasValidBuddyID() && !GetBuddyIP() && !GetBuddyPort() && !CGlobalVariable::serverconnect->IsLocalServer(GetServerIP(), GetServerPort()))
		{
			...
			if ( GetUploadState() == US_NONE && (!GetRemoteQueueRank() || m_bReaskPending) )
			{
                ...
                if( GetDownloadState() == DS_WAITCALLBACK )
				{
					if( GetBuddyIP() && GetBuddyPort())
					{
						...
						CGlobalVariable::clientudp->SendPacket(packet, GetBuddyIP(), GetBuddyPort(), false, NULL);  // kad doesnt supports obfuscation yet
...
CString CUpDownClient::GetDownloadStateDisplayString() const
{
	...
	switch (GetDownloadState())
	{
		...
		case DS_ONQUEUE:
			...
			break;
		case DS_DOWNLOADING:
            if(socket && socket->m_bUseNat) strState = GetResString(IDS_LOWTOLOWTRANSFERRING);
...
void CUpDownClient::SetCompletePartStatus( bool bComplete )
{
	...
	if( oldCount != m_nPartCount && m_abyPartStatus ) {
		// 如果前后不一致，那么删除掉原来的
		delete []m_abyPartStatus;
		m_abyPartStatus = NULL;
	}
