...
void CPartFile::CreatePartFile(const CString strPathfile, UINT /*cat*/)
{
	...
	for(int iFileSuffix = 0; ; )
	{
		...
		{
			if(! iFileSuffix) break;

			CString strNewFileName;
			strNewFileName.Format(_T("%s(%d).%s"), strName, iFileSuffix,strType);
		//	strNewFileName.Format(_T("%s(%d)"), GetFileName(), iFileSuffix);
    ...
    else
    {
        ...
        for (POSITION pos = srclist.GetHeadPosition(); pos != NULL;)
        {
			...
			case DS_LOWTOLOWIP:
				{
					...
					if ( cur_src->HasLowID() )
					{
						...
						if (! CGlobalVariable::DoCallback(cur_src) && !cur_src->IsSupportTraverse() )
						{
			...
			case DS_NONEEDEDPARTS:
				{
					if( (cur_src->m_iPeerType&ptINet)!=0 && cur_src->GetTimeUntilReask()==0 )
					{						
			...
			case DS_ERROR:
				{
					...
					if ( cur_src->IsKindOf(RUNTIME_CLASS(CHttpClient)) || cur_src->IsKindOf(RUNTIME_CLASS(CFtpClient)) )
					{
						if( GetPartFileSizeStatus()==FS_UNKNOWN && (GetTickCount()-m_dwTickGetFileSize)<5*1000 )
							break;
...
void CPartFile::PerformFileCompleteEnd(DWORD dwResult)
{
    if (dwResult & FILE_COMPLETION_THREAD_SUCCESS)
    {
		...
		CGlobalVariable::ShowNotifier(GetResString(IDS_TBN_DOWNLOADDONE) + _T('\n') + GetFileName(), TBN_DOWNLOADFINISHED, GetFilePath());
...
// VC-SearchDream[2007-05-21]: For download Sequencely Begin
bool CPartFile::GetNextRequestedBlockbySequence(CUpDownClient* sender,
...
// VC-SearchDream[2007-05-21]: For download Sequencely End
                                      ...
                                      uint16* count) /*const*/
{
    ...
    if (sender->IsKindOf(RUNTIME_CLASS(CHttpClient)) ||
            sender->IsKindOf(RUNTIME_CLASS(CFtpClient))) // VC-SearchDream[2007-03-21]: For HTTP and FTP Direct DownLoad
    {
		return GetNextRequestedBlockbySequence(sender, newblocks, count);
    }
...
void CPartFile::OnGetFileSizeFromInetPeer( uint64 uiFileSize )
{
	SetFileSize( uiFileSize );
	if( uiFileSize>0 )
		CreatePartFile( m_strDirectory );		
	hashsetneeded = false;
	m_PartFileSizeStatus = FS_KNOWN;
	//
...
/// TODO: 如何尽量考虑INet连续性,直接优先拿下一个连续的BlockRange？
INetBlockRange_Struct*	CPartFile::GetBlockRange(CUpDownClient* pClient)
{
	if( pClient==NULL )
		return NULL;
	
	if( pClient->m_pBlockRangeToDo )
		return pClient->m_pBlockRangeToDo; //考虑pause后resume,该Peer可以接着原来的BlockRange继续做..

	//ASSERT(pClient->m_pBlockRangeToDo==NULL);
		
	if( m_BlockRangeList.IsEmpty() )
	{
		SplitFileToBlockRange();
	}
	...
	for( POSITION pos=m_BlockRangeList.GetHeadPosition(); pos!=0; )
	{
		INetBlockRange_Struct* pBlockRange = m_BlockRangeList.GetNext(pos);
		if( pBlockRange->m_bFinished )
			continue;

		if( pBlockRange->m_pClient==NULL )
		{
			pBlockRange->m_pClient = pClient;
			pBlockRange->m_dwTakeOverTime = GetTickCount();
			pBlockRange->m_iBlockLastReqed = pBlockRange->m_iBlockCurrentDoing;
			pClient->m_pBlockRangeToDo = pBlockRange;
#ifdef _DEBUG_PEER
			Debug( _T("Peer(%d) Get the BlockRange(%d,%d,%d) because no peer do it.\n"),pClient->m_iPeerIndex,pBlockRange->m_iBlockIndexS
			...
			sLogOut.Format(GetResString(IDS_BLOCK_RANGE),pBlockRange->m_iBlockIndexS
				,pBlockRange->m_iBlockCurrentDoing,pBlockRange->m_iBlockIndexE );
			pClient->AddPeerLog(new CTraceInformation(sLogOut));
			return pBlockRange;
		}
	}
	...
	{
		INetBlockRange_Struct* pBlockRange = m_BlockRangeList.GetNext(pos);
		if( pBlockRange->m_bFinished /* ||pBlockRange->m_iBlockIndexE==pBlockRange->m_iBlockLastReqed */ )
			continue;		
		...
		if( pBlockRange->m_pClient->GetDownloadDatarate()==0 )
		{
			...
			pBlockRange->m_dwTakeOverTime = GetTickCount(); 
			pBlockRange->m_iBlockLastReqed = pBlockRange->m_iBlockCurrentDoing; 
			pBlockRange->m_pClient = pClient; /// 被这个新Source全部打劫
			pClient->m_pBlockRangeToDo = pBlockRange;
			CString sLogOut;
	...
	if( pClient->GetDownloadDatarate()==0 )
		iHijackCount = iBlockCountLeft / 2;
	else
		iHijackCount =  iBlockCountLeft*pClient->GetDownloadDatarate()/(pBlockRangeToHijack->m_pClient->GetDownloadDatarate() + pClient->GetDownloadDatarate());
	if( iHijackCount<1 )
	{
	...
	{
		//if( pBlockRangeToHijack->m_pClient->GetDownloadDatarate()==0 && iBlockCountLeft>1 ) //打劫速度为零的Peer
		...
		pNewBlockRange->m_pClient = pClient;
		pBlockRangeToHijack->m_iBlockIndexE = max(pNewBlockRange->m_iBlockIndexS-1,pBlockRangeToHijack->m_iBlockCurrentDoing); //被截尾
		if( iHijackCount==iBlockCountLeft )
		{
			pBlockRangeToHijack->m_bFinished = true; //全部被打劫了.. :-)
			pBlockRangeToHijack->m_pClient->SetDownloadState(DS_NONEEDEDPARTS);
		}
		m_BlockRangeList.AddTail( pNewBlockRange );
		pClient->m_pBlockRangeToDo = pNewBlockRange;
#ifdef _DEBUG_PEER
...
bool CPartFile::GetNextEmptyBlockInBlock( uint32 iBlockIdx, Requested_Block_Struct *pBlock,uint64& iContinuousEnd ) 
{
	...
	iBlockPosStart = (uint64)iPartIdx*PARTSIZE + (uint64)iTailBlockIdx*EMBLOCKSIZE;
	if( iTailBlockIdx==52 ) //Part的末尾一块只有140K
	{			
		iBlockPosEnd = iBlockPosStart + 140*1024 -1;
	}
	else
	{
		iBlockPosEnd   = iBlockPosStart + EMBLOCKSIZE-1 ;
	}
	iBlockPosEnd = min(iBlockPosEnd,m_nFileSize-(uint64)1);
...
void CPartFile::ConnectManage(CUpDownClient *pUpDownClient,uint32 uConnectIP,CClientReqSocket *pSocket)
{   
	int iConnectCount = 0;
	POSITION pos = srclist.GetHeadPosition();
	while(pos)
	{
       CUpDownClient *pClient = srclist.GetNext(pos);
       if(uConnectIP == pClient->m_nConnectIP && pClient->bNeedProcess == true)
		   iConnectCount++;
	}
	if(iConnectCount > 2)
	{
		pUpDownClient->bNeedProcess = false; 
		pSocket->Close();
