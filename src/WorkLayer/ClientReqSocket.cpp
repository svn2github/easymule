			...
			case OP_REQUESTSOURCES:
				{
					...
					if (client->GetSourceExchangeVersion() > 1)
					{
						...
						CKnownFile* reqfile;
						if ((reqfile = CGlobalVariable::sharedfiles->GetFileByID(packet)) != NULL ||
							(reqfile = CGlobalVariable::downloadqueue->GetFileByID(packet)) != NULL)
						{
			...
			case OP_REASKCALLBACKTCP:
				{
					...
					if (!reqfile)
					{
						...
						if (sender != NULL)
							CGlobalVariable::clientudp->SendPacket(response, destip, destport, sender->ShouldReceiveCryptUDPPackets(), sender->GetUserHash());
						else
							CGlobalVariable::clientudp->SendPacket(response, destip, destport, false, NULL);
						break;
					}
