...
void CSearchList::ShowResults(uint32 nSearchID)
{
	...
	SearchFileList * m_pSearchFileList = new SearchFileList;
	...
	for (POSITION pos = list.GetHeadPosition(); pos != NULL; )
	{
		...
		if (cur_file->GetListParent() == NULL && cur_file->GetSearchID() == nSearchID)
		{
			...
			m_pSearchFileList->AddTail(cur_file);
		}
	}

	SendMessage(CGlobalVariable::m_hListenWnd, WM_SEARCH_SHOWRESULT, 0, (LPARAM)m_pSearchFileList);
