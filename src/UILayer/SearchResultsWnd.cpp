...
bool CSearchResultsWnd::DoNewEd2kSearch(SSearchParams* pParams)
{
	...
	CGlobalVariable::searchlist->NewSearch(&searchlistctrl, strResultType, m_nEd2kSearchID, pParams->eType);
	canceld = false;
...
bool CSearchResultsWnd::DoNewKadSearch(SSearchParams* pParams)
{
	...
	if (strResultType == ED2KFTSTR_PROGRAM)
		strResultType.Empty();
	CGlobalVariable::searchlist->NewSearch(&searchlistctrl, strResultType, pParams->dwSearchID, pParams->eType);
	//CreateNewTab(pParams);
	searchlistctrl.ShowResults(pParams->dwSearchID);
	return true;
}
...
//		}
		...
		//SetItemCaption(GetResString(IDS_TABTITLE_SEARCH_RESULT) + str);
		...
		//MODIFIED by VC-fengwen on 2007/09/11 <begin> : StartSearch中有可能把pSearchParams删掉
			//StartSearch(pSearchParams);

			//CmdFuncs::UpdateResSearchParam(m_iCurSearchIndexInRes, pSearchParams);
		if (StartSearch(pSearchParams))
			CmdFuncs::UpdateResSearchParam(m_iCurSearchIndexInRes, pSearchParams);
		//MODIFIED by VC-fengwen on 2007/09/11 <end> : StartSearch中有可能把pSearchParams删掉
	}
}
