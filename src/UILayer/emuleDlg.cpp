...
void CemuleDlg::ProcessFileLink(LPCTSTR pszData)
{
	try {
		CString link2;
		CString link;
		...
		for(i = 0, j = _tcscspn(urls + i, _T("\r\n")); i < len; i += j + 1, j = _tcscspn(urls + i, _T("\r\n")))
		{
			if(j > 10)
			{
				...
				if(TestUrl(aurl))
				{
					...
					{
						CmdFuncs::AddUrlToDownload(pszData);
						return;
					}
...
LRESULT CemuleDlg::OnFileHashed(WPARAM wParam, LPARAM lParam)
{
	...
	else
	{
		...
		CGlobalVariable::sharedfiles->FileHashingFinished(result);
	}
	return TRUE;
}
...
LRESULT CemuleDlg::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SEARCH_SHOWRESULT:
		{
			SearchFileList * m_pSearchFileList = (SearchFileList*)lParam;
			...
			for (POSITION pos = m_pSearchFileList->GetHeadPosition(); pos != NULL; )
			{
				const CSearchFile* cur_file = m_pSearchFileList->GetNext(pos);
			...
			delete m_pSearchFileList; // 09/27/2007 Added by Soar Chin to resolve memory leaking
...
LRESULT CemuleDlg::OnStartED2KUpdate(WPARAM/* wParam*/, LPARAM/* lParam*/)
{
	...
	try
	{
		...
		case UPDATE_NODOWNLOAD:		//没有下载
			if( IDYES == MessageBox( GetResString(IDS_CAPTION) + GetResString(IDS_UPDATE_NEWVERSION), GetResString(IDS_CAPTION), MB_YESNO | MB_ICONQUESTION))
			{
		...
		case UPDATE_DOWNLOADED:		//已下载, 没有安装
			if( IDYES == MessageBox(GetResString(IDS_UPDATE_INSTALL), GetResString(IDS_CAPTION), MB_YESNO | MB_ICONQUESTION))
			{
				ShellExecute(NULL , _T("open"), CGlobalVariable::sharedfiles->GetFileByID(pLink->GetFileLink()->GetHashKey())->GetFilePath(), NULL,NULL,SW_SHOWNORMAL);
			}
			break;
		default:
			ASSERT(0);
...
void CemuleDlg::InitMenuIcon(void)
{
	...
	cif.AddIcon(MP_GETED2KLINK, theApp.LoadIcon(_T("ED2KLINK")));
	cif.AddIcon(MP_VIRUS, theApp.LoadIcon(_T("ANTIVIRUS")));
	...
	cif.AddIcon(MP_GETED2KLINK, theApp.LoadIcon(_T("ED2KLINK")), TRUE);
	cif.AddIcon(MP_VIRUS, theApp.LoadIcon(_T("ANTIVIRUS")), TRUE);
...
{
	m_mgrShortcuts.AddShortcut(TBBTN_HELP, VK_F1, 0);		//F1 help
