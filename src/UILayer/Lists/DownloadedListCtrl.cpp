	...
	if (   wParam == MP_CREATECOLLECTION || wParam == MP_FIND || wParam == MP_NEW/*|| wParam == MP_VIRUS*/ || selectedList.GetCount() > 0)
	{
		...
		switch (wParam){
			...
			//		file = selectedList.GetNext(pos);
					...
					if(pItemData->type == FILE_TASK)
					{
                       AfxMessageBox(GetResString(IDS_DELETEFILEINFO));
					}
				}
			...
			case IDA_ENTER:
				{
					...
					if(pItemData->type == FILE_TASK)
					{
						 ...
						 {
                            AfxMessageBox(GetResString(IDS_ZEROFILEINFO));
						 }
			...
			case MP_OPENFOLDER:
				{
					if(pItemData->type == FILE_TYPE)
					{
						...
						if(!PathFileExists(file->GetFilePath()))
						{
                           if(IDYES == AfxMessageBox(GetResString(IDS_OPENFOLDERINFO),MB_ICONWARNING | MB_ICONQUESTION | MB_DEFBUTTON2 | MB_YESNO))
...
//	LOGFONT lf = {0};
	...
	for(int iCurrent = 0; iCurrent < iCount; iCurrent++)
	{
		...
		if( !IsColumnHidden(iColumn))
		{
			...
			strItemCol = GetItemText(lpDrawItemStruct->itemID, iCurrent);
