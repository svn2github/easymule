			...
			case SDI_DIRECTORY:
				{
					...
					*/
					...
					if( iFileState==FILESTATE_SHARE || iFileState==FILESTATE_SHARE_TASK_DELED || iFileState==FILESTATE_LOCAL_SHARE )
						return;
					break;
				}				
			case SDI_INCOMING:
				{
					int iFileState = CGlobalVariable::filemgr.GetFileState(file->GetFileHash());
					if( iFileState!=FILESTATE_SHARE && iFileState!=FILESTATE_SHARE_TASK_DELED )
						return;
...
#ifdef _ENABLE_SHARE_FILES
	m_SharedFilesMenu.AppendMenu(MF_STRING,MP_UNSHAREFILE, GetResString(IDS_UNSHAREFILE), _T("DELETE"));
#endif
