...
void CDownloadListCtrl::DrawFileItem(CDC *dc, int nColumn, LPCRECT lpRect, CtrlItem_Struct *lpCtrlItem)
{
	if(lpRect->left < lpRect->right)
	{
	   ...
	   case 1:		
		...
		case 2:		// size			
			if ( lpPartFile->GetPartFileSizeStatus()==FS_KNOWN && lpPartFile->GetFileSize()>(uint64)0 )
			{
...
void CDownloadListCtrl::OnListModified(NMHDR* pNMHDR, LRESULT* /*pResult*/)
{
	...
	if (NULL == posSelected)
		theWndMgr.SendMsgTo(CWndMgr::WI_MAINTAB_DOWNLOAD_DLG, UM_MTDD_CUR_SEL_FILE, 0, NULL);
	else
	{
		const CtrlItem_Struct* content = (const CtrlItem_Struct*) GetItemData(GetNextSelectedItem(posSelected));
		if (NULL != content && FILE_TYPE == content->type)
		{
