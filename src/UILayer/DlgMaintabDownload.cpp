...
void CDlgMaintabDownload::InitTabs(void)
{
	...
	//添加上传页
	/*if (!m_dlgUpLoading.GetSafeHwnd())
	{
		m_dlgUpLoading.Create(m_dlgUpLoading.IDD, this);
	}*/
...
BOOL CDlgMaintabDownload::OnInitDialog()
{
	...
	AddAnchor(m_DownloadTabWnd, TOP_LEFT, CSize(100, thePrefs.GetSplitterbarPosition()));
	AddAnchor(m_wndSplitter, CSize(0, thePrefs.GetSplitterbarPosition()), BOTTOM_RIGHT);
	AddAnchor(m_tabwndInfo, CSize(0, thePrefs.GetSplitterbarPosition()), BOTTOM_RIGHT);
