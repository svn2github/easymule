...
BOOL CSplashScreen::OnInitDialog()
{
	...
	m_rtVersion.SetRect(m_rtImg.left + 209, m_rtImg.top + 293, m_rtImg.left + 253, m_rtImg.top + 311);
...
void CSplashScreen::DrawText(CDC *pDC)
{
	...
	CString strVer;
	strVer.Format(_T("V%u.%u"), CGlobalVariable::m_nVersionMjr, CGlobalVariable::m_nVersionMin);
	...
	font.CreateFontIndirect(&lf);
	pOldFont = pDC->SelectObject(&font);
	pDC->DrawText(_T("Copyright (C) 2002-2006 Merkur"), &m_rtText, DT_CENTER | DT_NOPREFIX);
	pDC->SelectObject(pOldFont);
	font.DeleteObject();
