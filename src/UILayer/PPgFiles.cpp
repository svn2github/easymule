...
BEGIN_MESSAGE_MAP(CPPgFiles, CPropertyPage)
	...
	ON_BN_CLICKED(IDC_MONITOR_ALT, OnSettingsChange) 
...
void CPPgFiles::LoadSettings(void)
{
	...
	CheckDlgButton(IDC_MONITOR_ALT, thePrefs.GetMonitorOnAlt() ? 1 : 0);
...
BOOL CPPgFiles::OnApply()
{
	...
	thePrefs.SetMonitorOnAlt(IsDlgButtonChecked(IDC_MONITOR_ALT) != 0);
...
void CPPgFiles::Localize(void)
{
	if (m_hWnd)
	{
		...
		GetDlgItem(IDC_MONITOR_ALT)->SetWindowText(GetResString(IDS_MONITOR_ALT));
...
{
	GetDlgItem(IDC_MONITOR_ALT)->EnableWindow(IsDlgButtonChecked(IDC_MONITOR_LINK) != 0);
	OnSettingsChange();
}