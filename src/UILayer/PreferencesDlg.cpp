...
void CPreferencesDlg::Localize()
{
	...
	if (pTree)
	{
		...
		if (m_bTipAdded)
		{
			m_ttc.UpdateTipText(GetResString(IDS_TIP_SCHEDULER), pTree, 6);
			m_ttc.UpdateTipText(GetResString(IDS_TIP_WEB_INTERFACE), pTree, 7);
...
void CPreferencesDlg::OnPageAdvanceChanged()
{
	if(IsDlgButtonChecked(s_unPageAdvId))
	{
		...
		if (!m_bTipAdded)
		{
			AddTreeItemTip(6, GetResString(IDS_TIP_SCHEDULER), 6);
			AddTreeItemTip(7, GetResString(IDS_TIP_WEB_INTERFACE), 7);
			m_bTipAdded = TRUE;
		}
	...
	else
	{
		...
		if(pSelectitem)
			if(GetPageTreeControl()->GetItemData(pSelectitem) > 4)
				GetPageTreeControl()->SelectItem(GetPageTreeItem(4));

		if(GetPageTreeControl()->GetCount() > 5)
		{
#if defined(_DEBUG) || defined(USE_DEBUG_DEVICE)
			HideTreeItem(11);
#endif
		...
		if (m_bTipAdded)
		{
			m_ttc.DelTool(GetPageTreeControl(), 6);
	...
	if(m_wndGeneral)
	{
		...
		m_wndGeneral.GetDlgItem(IDC_ED2KFIX)->ShowWindow(IsDlgButtonChecked(s_unPageAdvId)?SW_SHOW:SW_HIDE);
